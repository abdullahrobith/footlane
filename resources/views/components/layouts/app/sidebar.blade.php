...
<!-- HAPUS class="dark" agar tidak override -->
...
<body class="min-h-screen bg-zinc-100 text-zinc-900">
    <!-- Gunakan bg-zinc-100 untuk isi terang, atau bg-[#FFA500] untuk full oranye -->

    <!-- SIDEBAR -->
    <flux:sidebar sticky stashable
        class="border-r border-orange-500 bg-orange-500 text-white">
        ...
        <flux:navlist variant="outline" class="text-white">
            <flux:navlist.group :heading="__('menu')" class="grid">
                <flux:navlist.item icon="home" :href="route('dashboard')"
                    ...
                    wire:navigate
                    class="hover:bg-white hover:text-orange-500">
                    {{ __('Dashboard') }}
                </flux:navlist.item>
            ...
            <flux:menu class="w-[220px]">
                    ...
                    <div class="p-0 text-sm font-normal">
                        <div class="flex items-center gap-2 px-1 py-1.5 text-left text-sm">
                            <span class="relative flex h-8 w-8 shrink-0 overflow-hidden rounded-lg">
                                <span
                                    class="flex h-full w-full items-center justify-center rounded-lg bg-white text-orange-500">
                                    {{ auth()->user()->initials() }}
                                </span>
                            </span>

                            <div class="grid flex-1 text-left text-sm leading-tight">
                                <span class="truncate font-semibold text-orange-500">{{ auth()->user()->name }}</span>
                                <span class="truncate text-xs text-orange-500">{{ auth()->user()->email }}</span>
                            </div>
                        </div>
                    </div>
                </flux:menu.radio.group>
                ...
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    ...
                    <flux:menu.item as="button" type="submit"
                        ...
                        class="w-full text-orange-500 hover:bg-orange-100">
                        {{ __('Log Out') }}
                    </flux:menu.item>
                </form>
            </flux:menu>
    ...
    <!-- Mobile User Menu -->
    <flux:header class="lg:hidden bg-orange-500 text-white">
