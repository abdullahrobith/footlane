                            ...
                            @forelse($cart->items as $item)
                                @if($item->itemable)
                                    <div class="cart-item d-flex align-items-center mb-3 border-bottom pb-3">
                                        <img src="{{ $item->itemable->image_url ?? 'https://via.placeholder.com/80' }}"
                                            class="cart-img me-3 rounded" alt="{{ $item->itemable->getName() }}">
                                        <div class="flex-grow-1">
                                            <h5 class="cart-item-name mb-1">{{ $item->itemable->getName() }}</h5>
                                            <p class="cart-item-price mb-0 text-muted">Rp.
                                                {{ number_format($item->itemable->getPrice(), 0, ',', '.') }}</p>
                                        </div>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <span class="cart-item-subtotal mb-0 me-3 fw-semibold">
                                                Rp.{{ number_format($item->itemable->getPrice() * $item->quantity, 0, ',', '.') }}
                                            </span>
                                ...
                                @else
                                    <div class="alert alert-warning d-flex justify-content-between align-items-center">
                                        <div>Produk tidak ditemukan atau telah dihapus.</div>
                                        <form action="{{ route('cart.remove', $item->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
